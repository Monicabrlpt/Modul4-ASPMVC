    ...
    public class UserManager
    {
        ...
        public string GetUserPassword(string loginName)
        {
            using (DemoDBEntities db = new DemoDBEntities())
            {
                ...
                if (user.Any()) return user.FirstOrDefault().PasswordEncryptedText;
            ...
            using (DemoDBEntities db = new DemoDBEntities())
            {
                ...
                if (SU != null)
                {
                    var roles = from q in db.SYSUserRoles join r in db.LOOKUPRoles on q.LOOKUPRoleID equals r.LOOKUPRoleID where r.RoleName.Equals(roleName) && q.SYSUserID.Equals(SU.SYSUserID)
                                select r.RoleName; if (roles != null) { return roles.Any();
                    }
                }
                return false;
            }
        ...
        public List<LOOKUPAvailableRole> GetAllRoles()
        {
            using (DemoDBEntities db = new DemoDBEntities())
            {
                ...
                { LOOKUPRoleID = o.LOOKUPRoleID, RoleName = o.RoleName, RoleDescription = o.RoleDescription }).ToList();
                return roles;
            }
        }
        ...
        public List<UserProfileView> GetAllUserProfiles()
        {
            ...
            using (DemoDBEntities db = new DemoDBEntities())
            {
                UserProfileView UPV; var users = db.SYSUsers.ToList(); foreach (SYSUser u in db.SYSUsers)
                {
        ...
        public UserDataView GetUserDataView(string loginName)
        {
            ...
            int? userAssignedRoleID = 0, userID = 0; string userGender = string.Empty;
                ...
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (user.LOOKUPRoleID > 0)
                        {
                            ...
                            SYSUserRole SUR = null; if (userRole.Any())
                            {
            ...
            using (DemoDBEntities db = new DemoDBEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        var SU = db.SYSUsers.Where(o => o.SYSUserID == userID); if (SU.Any())
                        {
